import { TOONEncoder } from '../src/toon-encoder.js';
...
for (const [key, dataset] of Object.entries(datasets)) {
    ...
    console.log(dataset.name.padEnd(25) +
        json.toString().padEnd(12) +
        toon.toString().padEnd(12) +
        `${savings.toFixed(1)}%`);
}
// Performance benchmarks
console.log('\n⏱️  Performance Benchmarks (ms for ' + ITERATIONS + ' iterations)\n');
console.log('Dataset'.padEnd(25) + 'JSON.stringify'.padEnd(18) + 'TOON.encode'.padEnd(18) + 'Ratio');
console.log('-'.repeat(80));
for (const [key, dataset] of Object.entries(datasets)) {
    const jsonTime = benchmark('JSON.stringify', () => JSON.stringify(dataset.data), ITERATIONS);
    const toonTime = benchmark('TOON.encode', () => TOONEncoder.encode(dataset.data), ITERATIONS);
...
// Decoding benchmarks
console.log('\n⏱️  Decoding Benchmarks (ms for ' + ITERATIONS + ' iterations)\n');
...
for (const [key, dataset] of Object.entries(datasets)) {
    ...
    const jsonTime = benchmark('JSON.parse', () => JSON.parse(jsonStr), ITERATIONS);
    const toonTime = benchmark('TOON.decode', () => TOONEncoder.decode(toonStr), ITERATIONS);
    const ratio = (toonTime / jsonTime).toFixed(2);
    console.log(dataset.name.padEnd(25) +
        jsonTime.toFixed(2).padEnd(18) +
        toonTime.toFixed(2).padEnd(18) +
